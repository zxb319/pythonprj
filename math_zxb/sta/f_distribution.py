...
class FDistribution(Dist):
    def __init__(self,m:int,n:int):
        ...
        def f(x:float):
            ...
            b=math.gamma(m/2)*math.gamma(n/2)
            return a/b
    ...
    def x_of(self,p:float):
        assert 0<p<1
        return math_zxb.root_binarily(lambda x:self.cp(x)-p,0,1000)

    def range_of(self,p:float):
        lo=math_zxb.root_binarily(lambda x:self.cp(x)-(1-p)/2,0,1000)
        hi=math_zxb.root_binarily(lambda x:self.cp(x)-(1+p)/2,0,1000)

        return lo,hi


if __name__=='__main__':
    pd=FDistribution(9,7)
    print(pd.x_of(0.05))
